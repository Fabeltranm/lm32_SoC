//--------------------------------------------------------------------------------
// Auto-generated by Migen (54d666d) & LiteX (f2570701) on 2019-06-17 10:48:03
//--------------------------------------------------------------------------------
module top(
...
reg button_old_trigger = 1'd0;
wire button_status_re;
wire button_status_r;
wire button_status_w;
wire button_pending_re;
wire button_pending_r;
wire button_pending_w;
reg button_storage_full = 1'd0;
wire button_storage;
reg button_re = 1'd0;
...
assign led010 = led_storage;
assign button_status_w = button_status;
always @(*) begin
	button_clear <= 1'd0;
...
assign button_pending_w = button_pending;
assign button_irq = (button_pending_w & button_storage);
...
assign button_status_r = interface0_bank_bus_dat_w[0];
assign button_status_re = ((csrbank0_sel & interface0_bank_bus_we) & (interface0_bank_bus_adr[1:0] == 1'd0));
assign button_pending_r = interface0_bank_bus_dat_w[0];
assign button_pending_re = ((csrbank0_sel & interface0_bank_bus_we) & (interface0_bank_bus_adr[1:0] == 1'd1));
assign csrbank0_ev_enable0_r = interface0_bank_bus_dat_w[0];
assign csrbank0_ev_enable0_re = ((csrbank0_sel & interface0_bank_bus_we) & (interface0_bank_bus_adr[1:0] == 2'd2));
assign button_storage = button_storage_full;
assign csrbank0_ev_enable0_w = button_storage_full;
...
assign led_storage = led_storage_full;
assign csrbank2_out0_w = led_storage_full;
...
always @(posedge sys_clk) begin
	...
	button_old_trigger <= button_trigger;
	if (((~button_trigger) & button_old_trigger)) begin
		button_pending <= 1'd1;
	end
	case (grant)
		1'd0: begin
	...
	if (csrbank0_sel) begin
		case (interface0_bank_bus_adr[1:0])
			1'd0: begin
				interface0_bank_bus_dat_r <= button_status_w;
			end
			1'd1: begin
				interface0_bank_bus_dat_r <= button_pending_w;
			end
			2'd2: begin
	...
	if (csrbank0_ev_enable0_re) begin
		button_storage_full <= csrbank0_ev_enable0_r;
	end
	button_re <= csrbank0_ev_enable0_re;
	...
	if (csrbank2_out0_re) begin
		led_storage_full <= csrbank2_out0_r;
	end
	led_re <= csrbank2_out0_re;
	...
	if (sys_rst) begin
		...
		timer0_value <= 32'd0;
		led_storage_full <= 1'd0;
		led_re <= 1'd0;
		button_pending <= 1'd0;
		button_old_trigger <= 1'd0;
		button_storage_full <= 1'd0;
		button_re <= 1'd0;
		grant <= 1'd0;
		slave_sel_r <= 4'd0;
		count <= 20'd1000000;
